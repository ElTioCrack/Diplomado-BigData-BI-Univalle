...
def clasificar_severidad(row):
    ...
    criticas = ["no aparece", "error", "bloqueo", "desaparecieron", "rechazaron",
                "ansiedad", "incertidumbre", "caos", "nadie resuelve", "desgasta",
                "dudando", "maratón", "cadena de confusiones"]
    
    # Lo superficial
    esteticas = ["linda", "bonito", "diseño", "interfaz", "color", "carga bien"]
    
    score = sum(1 for p in criticas if p in texto) - sum(1 for p in esteticas if p in texto)
...
palabras_filtradas = {k: v for k, v in palabras.items() 
                     if len(k) > 3 and k not in ['para', 'una', 'como', 'pero', 
                                                 'soy', 'mi', 'la', 'el', 'que',
                                                 'los', 'las', 'del', 'por', 'con']}
...
gs = gridspec.GridSpec(3, 3, height_ratios=[1.2, 1, 0.9], 
                       width_ratios=[1, 0.9, 1],
                       hspace=0.35, wspace=0.3)

# --- Título con personalidad ---
fig.suptitle('El problema no es el campus\nes cómo llegamos hasta él', 
             fontsize=20, fontweight='bold', color=MI_PALETA['texto_oscuro'],
             y=0.97, linespacing=1.4)
fig.text(0.02, 0.94, 'Análisis de 80 conversaciones | Inicio de semestre 2026', 
         fontsize=11, color=MI_PALETA['gris_calido'], style='italic')
...
colores_barras = [MI_PALETA['rojo_principal'] if i == len(temas_ordenados)-1 
                  else MI_PALETA['azul_profundo'] for i in range(len(temas_ordenados))]

bars = ax1.barh(temas_ordenados.index, temas_ordenados.values, 
                color=colores_barras, height=0.6, edgecolor='white', linewidth=1.5)

ax1.set_xlabel('Número de comentarios', fontsize=10, color=MI_PALETA['texto_claro'])
ax1.set_title('¿De qué estamos hablando?', fontsize=13, fontweight='bold', 
              color=MI_PALETA['texto_oscuro'], pad=15)
...
wedges, texts, autotexts = ax2.pie(tonos.values, labels=labels, 
...
# Texto en el centro
ax2.text(0, 0.15, f"{tonos.index[0]}", fontsize=14, fontweight='bold', 
         ha='center', color=MI_PALETA['texto_oscuro'])
ax2.text(0, -0.15, f"{tonos.values[0]} comentarios", fontsize=10, 
         ha='center', color=MI_PALETA['texto_claro'])

ax2.set_title('El estado de ánimo', fontsize=13, fontweight='bold', 
              color=MI_PALETA['texto_oscuro'], pad=15)
...
bars3 = ax3.bar(severidad_counts.index, severidad_counts.values, 
                color=colores_sev, edgecolor='white', linewidth=2, width=0.55)

ax3.set_ylabel('Incidentes', fontsize=10, color=MI_PALETA['texto_claro'])
ax3.set_title('¿Qué tan grave es?', fontsize=13, fontweight='bold', 
              color=MI_PALETA['texto_oscuro'], pad=15)
...
# Línea de alerta
ax3.axhline(y=severidad_counts['Alta'], color=MI_PALETA['rojo_principal'], 
            linestyle='--', alpha=0.5, linewidth=1.5)
ax3.text(2.5, severidad_counts['Alta'] + 0.5, '⚠ umbral crítico', 
         fontsize=8, color=MI_PALETA['rojo_principal'], style='italic')
...
bars4 = ax4.bar(canales.index, canales.values, color=colores_canal, 
...
ax4.set_title('¿Por dónde se filtra?', fontsize=13, fontweight='bold', 
              color=MI_PALETA['texto_oscuro'], pad=15)
...
colores_palabras = [MI_PALETA['rojo_principal'] if i < 3 else MI_PALETA['azul_profundo'] 
...
for i, (palabra, freq) in enumerate(palabras_mostrar):
    ax5.text(0.5, y_pos[i], f'"{palabra}"', fontsize=tamaños[i], 
             fontweight='bold', color=colores_palabras[i],
             ha='center', transform=ax5.transAxes,
             bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                      edgecolor=colores_palabras[i], alpha=0.7))

ax5.text(0.5, 0.95, 'El vocabulario de la frustración', fontsize=12, 
         fontweight='bold', ha='center', color=MI_PALETA['texto_oscuro'],
         transform=ax5.transAxes)
ax5.text(0.5, 0.90, '(lo que más se repite en quejas críticas)', fontsize=9, 
         ha='center', color=MI_PALETA['gris_calido'], style='italic',
         transform=ax5.transAxes)
...
sns.heatmap(cross_norm, annot=True, fmt='.0f', cmap='Reds',
            ax=ax6, cbar_kws={'label': '%', 'shrink': 0.8},
            linewidths=1, linecolor='white', square=False,
            annot_kws={'size': 9, 'weight': 'bold'},
            vmin=0, vmax=100)

ax6.set_title('¿Qué temas duelen más?', fontsize=13, fontweight='bold', 
              color=MI_PALETA['texto_oscuro'], pad=15)
...
rect = FancyBboxPatch((0.05, 0.1), 0.9, 0.7, 
                      boxstyle="round,pad=0.3",
                      facecolor='white', edgecolor=MI_PALETA['rojo_principal'],
                      linewidth=2.5, alpha=0.9, transform=ax7.transAxes)
ax7.add_patch(rect)

# Texto de la conclusión
ax7.text(0.5, 0.65, conclusion[0], fontsize=16, fontweight='bold',
         ha='center', color=MI_PALETA['texto_oscuro'], transform=ax7.transAxes)
ax7.text(0.5, 0.50, conclusion[1], fontsize=16, fontweight='bold',
         ha='center', color=MI_PALETA['rojo_principal'], transform=ax7.transAxes)

for i, linea in enumerate(conclusion[3:]):
    ax7.text(0.15, 0.35 - i*0.08, linea, fontsize=11, 
             color=MI_PALETA['texto_claro'], transform=ax7.transAxes)

# Flecha o símbolo de atención
ax7.text(0.92, 0.70, '⚠', fontsize=24, transform=ax7.transAxes, 
...
plt.savefig('dashboard_problema_real.png', dpi=350, bbox_inches='tight',
...
print(f"   • {top_temas.index[0]} es el tema más mencionado ({top_temas.iloc[0]} comentarios)")
print(f"   • Parece un problema de tecnología")

print(f"\n🎯 ¿Qué está pasando realmente?")
print(f"   • ES UN PROBLEMA DE COORDINACIÓN")
print(f"   • {severidad_counts['Alta']} incidentes críticos son por falta de comunicación entre áreas")
